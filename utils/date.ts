...
module.exports = {
  ...
  weekRecordInitChecking: async (id: number, krToday: krToday) => {
    ...
    const numLUZ = Number(lastUpdatedDate);
    ...
    if (checkingZeroHour.getDay() === 0) {
      ...
      // 월요일 ~ 토요일
      if (
        numLUZ <= numCZH - checkingZeroHour.getDay() * oneDay ||
        numCZH + (8 - checkingZeroHour.getDay()) * oneDay <= numLUZ
      ) {
        // 요일 초기화 실행
        await User.update({ lastUpdated: checkingDate }, { where: { id } });
  ...
  monthRecordInitChecking: async (id: number, krToday: krToday) => {
