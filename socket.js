...
io.on("connection", (socket) => {
  ...
  //참가자 관전자 안나눈거
  socket.on("join room", async (roomID, nickname, done) => {
    if (!roomID || !nickname) {
    ...
    console.log("현재", roomID, "방안에 있는 인원 수는", users[roomID].length);
    socket.join(roomID);
    console.log(io.sockets.adapter.rooms);
    ...
    // socket.to(data.roomID)  //roomID안에 모든사람(나 자신 제외)
    // io.sockets.in(data.roomID)  //roomID안에 모든사람 (나 자신 포함)
    socket.emit("room users", otherInRoom);
  });
