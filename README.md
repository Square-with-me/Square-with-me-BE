...
#### - 주간, 월간 기록 초기화
1. 문제 상황: 이번 주 혹은 이번 달 참여 시간을 보여주어야 하는데 주 혹은 월이 바뀌면 기록이 초기화 되어야 합니다.
2. 해결방법: 월간의 경우 마지막 업데이트 시점과 현재 조회 시점을 체크하여 서로 다른 달에 속하거나 같은 달에 속하더라도 연도가 다르다면 초기화하였습니다. 주간 역시 동일한 방식을 적용하려 했지만 특정 연도가 1월 1일에 시작되는 경우에만 정상작동하는 알고리즘이었습니다. 이를 해결하고자 현재 시점으로 부터 마지막 업데이트 시점이 며칠이 떨어져 있는지 체크하였습니다. 마지막 업데이트 날이 지난주 일요일이고 현재 조회하는 시점이 이번주 일요일이라고 가정하면 마지막업데이트 "시점 ≤ 오늘 - 86400000(하루를 ms로 나타낸 수치) X 7" 일 때 주가 바뀌었다고 판단할 수 있습니다. 이러한 식을 요일별로 다르게 나타내어 일종의 공식을 구현했습니다. 다만 여기선 요일만 비교하기에 두 시점의 시, 분, 초를 모두 0으로 초기화 후 진행하였습니다.
</br>
